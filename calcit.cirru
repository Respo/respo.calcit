...
{} (:about "|Machine-generated snapshot. Do not edit directly — changes will be overwritten. Use `cr query` to inspect and `cr edit`/`cr tree` to modify. Run `cr docs agents --full` first. Manual edits must follow format and schema conventions, then run `cr edit format`.") (:package |respo)
  :configs $ {} (:init-fn |respo.main/main!) (:reload-fn |respo.main/reload!) (:version |0.16.52)
    ...
    |respo.core $ %{} :FileEntry
      :defs $ {}
        ...
        |create-list-element $ %{} :CodeEntry (:doc "|Creates a virtual DOM element for list rendering. Arguments: tag-name, props, child-map (map of key -> child).")
          ...
          :schema $ :: :fn
            {} (:return 'respo.schema/Element)
              :args $ [] :tag (:: :optional 'respo.schema/DomProps) :map
        ...
        |pick-attrs $ %{} :CodeEntry (:doc "|Extracts HTML attributes from a properties map, filtering out internal keys like :on, :event, :style.")
          :code $ quote
            defn pick-attrs (props)
              if (nil? props) ([])
                -> props (&map:dissoc :on) (&map:dissoc :style) (&map:to-list)
                  filter $ fn (pair)
                    let
                        k $ nth pair 0
                        v $ nth pair 1
